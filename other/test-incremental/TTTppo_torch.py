...
#ppo memory class for incremental learning
class PPOMemory:
...
#actor network class for incremental learning
...
#critic network class for incremental learning
...
#ppo agent class for incremental learning
class PPOAgent:
    def __init__(
        self,
        n_actions,
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for t in reversed(range(len(reward_arr))):
                delta = reward_arr[t] + self.gamma * values[t + 1] * (1 - done_arr[t]) - values[t]
                gae = delta + self.gamma * self.gae_lambda * (1 - done_arr[t]) * gae
                advantage[t] = gae

            advantage = T.tensor(advantage).to(self.actor.device)
            old_prob_arr = T.tensor(old_prob_arr).to(self.actor.device)
